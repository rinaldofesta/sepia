...
Current state: the engine reproduces Inkling's forward pass token-exactly against the transformers reference, on CPU, gated in CI on every push. P1 is done: it now generates real greedy tokens from the full 317GB weights, cross-checked against a llama.cpp build on the same GGUF (text-exact on both test prompts, id-exact on one). P2 is done: resident weights, banded attention, and MoE routing moved to Metal, with routed experts served from an LRU-streamed, mlocked GPU cache and async prefetch overlap. Real measured throughput: ~1.39-1.57 tok/s steady-state on both P1 prompts, cold and warm (down from P1's ~29-31s/token, i.e. ~0.6-0.7s/token now) — at the floor of the project's original 1.5-3 tok/s target, and the binding constraint is now GPU compute, not disk I/O (measured tok/s falls short of even the cold, 0%-hit pure-I/O ceiling). P2's own routing-predictability experiment (PILOT) published a clean negative result: mean one-layer-ahead recall@6 is 2.34%, indistinguishable from the 2.34% random-chance baseline and far below both the ~60% P3-viability threshold and colibrì's 71.6% GLM-5.2 measurement — Inkling's router is not predictable one layer ahead. A separate measurement, same-layer consecutive-token temporal locality (38.2%, ~16x chance), is real and is what the expert cache's hit rates above actually exploit — a different mechanism from what PILOT prefetch targets. Full results: [docs/p2-perf.md](docs/p2-perf.md) and [docs/pilot-routing.md](docs/pilot-routing.md); P1's correctness baseline: [docs/p1-first-tokens.md](docs/p1-first-tokens.md).
...
| PILOT, one-layer-ahead expert recall@6 (708 tokens, 4 prompts) | 2.34% (n=44,604; ~= 2.34% random-chance baseline) | [pilot-routing](docs/pilot-routing.md) |
| PILOT, same-layer consecutive-token expert overlap | 38.2% (~16x chance) | [pilot-routing](docs/pilot-routing.md) |
...
- P1 (done): 7 dequant types bit-exact vs gguf-py fixtures (CI-gated); C tokenizer exact vs HF tokenizers (mini 9/9, real 25/25, ~8300-string stress sweep via [tools/stress_tokenizer.py](tools/stress_tokenizer.py), zero mismatches) and vs llama-tokenize (7/7); first real greedy tokens deterministic on both test prompts, cross-checked against the pinned llama.cpp draft-PR build (text-exact both prompts, id-exact on prompt 1); ~29-31s/token steady on the scalar CPU path, correctness-only. Full results: [docs/p1-first-tokens.md](docs/p1-first-tokens.md)
- P2 (done): resident weights, banded attention, and MoE routing on Metal; routed experts served from an LRU-streamed, mlocked GPU cache with async prefetch overlap; both P1 prompts still exact-sequence on GPU. Measured throughput ~1.39-1.57 tok/s steady-state (cold and warm) — at the floor of the 1.5-3 tok/s target, and now compute-bound rather than I/O-bound (falls short of even the cold pure-I/O ceiling despite good cache hit rates). Full results: [docs/p2-perf.md](docs/p2-perf.md). Routing-predictability experiment (PILOT) published a clean negative result: mean one-layer-ahead recall@6 = 2.34% vs a ~60% P3-viability threshold and colibrì's 71.6% GLM-5.2 baseline — the router is not predictable one layer ahead. Same-layer consecutive-token temporal locality is real (38.2%, ~16x chance) and is what P2's own cache hit rates exploit. Full results: [docs/pilot-routing.md](docs/pilot-routing.md).
- P3: learning cache (`.sepia_usage`) and confidence-ramped auto-pin, heat/hysteresis re-pin — these exploit the temporal locality PILOT confirmed is real, independent of PILOT's own next-layer-prefetch question. PILOT *prefetch* specifically is now explicitly not justified by the P2 measurement above and is dropped from P3's scope.
