...
- ci gains `shadercheck`: offline-compiles every `metal/*.metal` (`xcrun -sdk macosx metal -c`) **when the offline Metal compiler exists** (`xcrun -f metal`); when absent (Command Line Tools-only machines have the framework but not the offline compiler) it prints `shadercheck: skipped (no offline Metal compiler; runtime compile covered by --metal)` and exits 0 — the macos-latest runner has full Xcode, so CI always genuinely checks. [Amended 2026-07-20: hard-failing locally without Xcode.app violated the make-ci-green invariant.]
