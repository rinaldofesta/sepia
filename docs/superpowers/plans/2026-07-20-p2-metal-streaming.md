...
- ci gains `shadercheck`: **concatenates `metal/*.metal` in sorted order into one temporary file — the exact translation unit `sepia_gpu_load_source` builds at runtime — and offline-compiles THAT** (`xcrun -sdk macosx metal -c`) when the offline Metal compiler exists (`xcrun -f metal`); when absent (Command Line Tools-only machines have the framework but not the offline compiler) it prints `shadercheck: skipped (no offline Metal compiler; runtime compile covered by --metal)` and exits 0 — the macos-latest runner has full Xcode, so CI always genuinely checks. Individual `.metal` files additionally carry `extern constant` forward declarations for any cross-file symbols they consume, so they remain self-documenting and per-file-compilable. [Amended 2026-07-20 twice: (1) hard-failing locally without Xcode.app violated the make-ci-green invariant; (2) per-file compilation diverged from the runtime's concatenated-TU model once cross-file grid constants existed — Task 6 review caught the latent CI break.]
