 ...
 * block-aligned preads of a fixed size against one file, buffered or
 * F_NOCACHE (macOS only -- this tool has no other target).
 *
 * Usage: ./iobench <file> <block_mb> <n_reads> <threads> <direct 0|1>
 *   block_mb : read size in MiB (block-aligned offsets within the file)
...
static void *worker(void *argp) {
    ...
    for (long i = 0; i < a->n_reads; i++) {
        long idx = (long)arc4random_uniform((uint32_t)a->num_blocks);
        off_t offset = (off_t)idx * (off_t)a->block_bytes;
        double t0 = now_ms();
...
int main(int argc, char **argv) {
    ...
    long num_blocks = (long)((size_t)st.st_size / block_bytes);
    if (num_blocks < 1) {
        fprintf(stderr,
    ...
    printf("file=%s block_mb=%ld n_reads=%ld threads=%ld direct=%d\n", path,
           block_mb, n_reads, threads, direct);
